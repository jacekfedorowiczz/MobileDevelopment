...
{
    public sealed record GetPagedDietDaysQuery(int DietId, int PageNumber = 1, int PageSize = 10) : IRequest<Result<PagedResult<DietDayDto>>>;
    ...
    public sealed class GetPagedDietDaysQueryHandler(IDietDayService dietDayService) : IRequestHandler<GetPagedDietDaysQuery, Result<PagedResult<DietDayDto>>>
    {
        public Task<Result<PagedResult<DietDayDto>>> Handle(GetPagedDietDaysQuery request, CancellationToken cancellationToken)
        {
            return dietDayService.GetPagedByDietIdAsync(request.DietId, request.PageNumber, request.PageSize, cancellationToken);
        }
    }
}